async function run() {
    console.log("Generating Tablet Layout...");

    const page = await framer.createDesignPage("Tablet Site");
    ...
    await framer.createTextNode({ text: "& DESIGN\nENGINEER", top: 120, right: 40, position: "absolute", textAlign: "right", ...txtStyle }, about.id);
    ...
    await framer.createTextNode({ text: "CULTURAL\n& CREATIVE", bottom: 120, left: 40, position: "absolute", ...txtStyle }, about.id);
    ...
    await framer.createTextNode({ text: "BHX", top: 40, right: 250, position: "absolute", ...txtStyle }, location.id);
    ...
    await framer.createTextNode({ 
        text: "LET'S BUILD\nOR CHAT\nABOUT\nSOMETHING", 
        fontSize: 55, letterSpacing: -2.75, lineHeight: 0.9, textAlign: "center"
    }, footer.id);

    console.log("Tablet layout done!");
}
